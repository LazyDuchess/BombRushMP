                ...
                switch (packetId)
                {
                    ...
                    case Packets.ClientComboOver:
                        if (ComboBased)
                        {
                            if (!_clientsFinishedCombo.Contains(playerId))
                            {
                                ServerLobbyManager.SendPacketToLobby(new ServerChat(TMPFilter.CloseAllTags(BRCServer.Instance.Players[playerId].ClientState.Name), "{0} landed their combo!", BRCServer.Instance.Players[playerId].ClientState.User.Badge, ChatMessageTypes.System), IMessage.SendModes.ReliableUnordered, Lobby.LobbyState.Id, NetChannels.Gamemodes);
